...
void Renderer::initialize() {
    ...
    // Only create the radix sort pass if the GL implementation exposes the
    // subgroup extensions required by the compute shaders (used by the
    // RadixSort implementation). If not available, skip creating the pass
    // to avoid runtime shader compile errors on older drivers.
    if (glUtils::supportsShaderSubgroupExtensions()) {
        renderPasses[radixSortPassName] = std::make_unique<RadixSortPass>();
    } else {
        std::cerr << "Warning: GPU does not support required shader subgroup extensions; radix sort pass disabled." << std::endl;
    }
...
void Renderer::renderFrame()
{
    ...
    {
        // Avoid operator[] here because some passes (e.g. radixSort) may not
        // have been created when the GPU lacks required extensions. Use
        // find() and validate the pointer before calling into the pass.
        auto it = renderPasses.find(renderPassName);
        if (it == renderPasses.end())
            continue;

        auto& passPtr = it->second;
        if (!passPtr)
            continue;
