...
namespace glUtils 
{
    ...
    bool supportsShaderSubgroupExtensions()
    {
        // Query for the presence of the required GL extension tokens. We
        // check for both the KHR and EXT variants commonly used on drivers.
        ...
        bool hasKHR = (extStr.find("GL_KHR_shader_subgroup_arithmetic") != std::string::npos) &&
                      (extStr.find("GL_KHR_shader_subgroup_shuffle_relative") != std::string::npos);
        bool hasEXT = (extStr.find("GL_EXT_shader_subgroup_vote") != std::string::npos) ||
                      (extStr.find("GL_EXT_shader_subgroup_uniform_vote") != std::string::npos);

        return hasKHR || hasEXT;
    }
