...
{ 
    ...
    // Returns true if required GL_KHR subgroup extensions for the GPU compute
    // shaders are available on the current GL context. This is used to
    // determine whether advanced compute-based radix sort / scan shaders
    // (which depend on subgroup operations) can be used.
